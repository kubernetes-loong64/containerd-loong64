...
推送 `loong64-<containerd 版本>` 格式的分支（如 `loong64-v2.3.5`）即可触发构建。可追加 `+<build>`
（如 `loong64-v2.3.5+0`）携带构建元数据。

## [发布](https://github.com/kubernetes-loong64/containerd-loong64/releases)

推送 `release-loong64-<containerd 版本>` 格式的标签（如 `release-loong64-v2.3.5+0`）即可自动创建 GitHub
Release 并上传构建产物。
